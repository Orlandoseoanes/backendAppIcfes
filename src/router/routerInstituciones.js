...
router.post('/Instituciones', async (req, res) => {
    ...
    if (isNaN(Nit_institucion) || Nit_institucion < 10000 || Nit_institucion > 99999999) {
        return res.status(400).json({ message: 'El NIT de la institución debe ser numérico y estar entre 10000 y 99999999' });
    }

    // Validación 2: Verificar que Nombre_Institucion sea una cadena de texto y cumpla con los límites
    if (typeof Nombre_Institucion !== 'string' || Nombre_Institucion.length < 2 || Nombre_Institucion.length > 40) {
        return res.status(400).json({ message: 'El nombre de la institución debe ser una cadena de texto válida entre 2 y 40 caracteres' });
    }

    try {
        // Verificar si ya existe una institución con el mismo NIT
        const existingInstitucion = await ModeloInstitucion.findOne({ where: { Nit_institucion } });
        if (existingInstitucion) {
            return res.status(409).json({ message: 'Institución ya existe, por favor ingrese una nueva' });
        }
        ...
        if (error.name === 'SequelizeUniqueConstraintError' && error.fields && 'PRIMARY' in error.fields) {
            return res.status(409).json({ message: 'Institución ya existe, por favor ingrese una nueva' });
        }
        res.status(500).json({
