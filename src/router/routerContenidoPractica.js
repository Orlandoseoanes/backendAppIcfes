...
router.post("/ContentPractice/Register", async (req, res) => {
  try {
    ...
    // Validación de existencia del ID de práctica
    const practiceExists = await ModelPractice.findByPk(IdPractica);
    if (!practiceExists) {
      return res
        .status(400)
        .json({ error: "La práctica especificada no existe." });
    }
    // Validación de tipo numérico del ID de práctica
    if (isNaN(IdPractica)) {
      return res
        .status(400)
        .json({ error: "El ID de práctica debe ser numérico." });
    }
    if (!Preguntas || Preguntas.length === 0) {
      return res
        .status(400)
        .json({ error: "Tiene que haber al menos 1 pregunta." });
    }
    if (
      !Array.isArray(Preguntas) ||
      Preguntas.some((question) => typeof question !== "string")
    ) {
      return res
        .status(400)
        .json({ error: "El campo Preguntas debe ser un array de cadenas de caracteres." });
    }

    let questionsArray = [];

    for (const questionId of Preguntas) {
      const questionExists = await QuestionBankModel.findById(questionId);
      if (!questionExists) {
        return res
          .status(400)
          .json({ error: `La pregunta con ID ${questionId} no existe.` });
      }
      questionsArray.push(questionExists);
    }

    const { nombre } = practiceExists;
...
router.post("/ContentPractice/Evaluate", async (req, res) => {
  try {
    ...
    if (!Respuestas || Respuestas.length === 0) {
      return res
        .status(400)
        .json({ error: "El array de Respuestas no puede estar vacío." });
    }
...
router.get(
  "/ContentPractice/GetLogPractices/student/:Cedula",
  async (req, res) => {
    try {
      const { Cedula } = req.params;
      const cedulaNumber = parseInt(Cedula, 10);
      const logPractices = await ModeloLogPracticas.find({
        "Alumno.Documento": cedulaNumber,
      });
      ...
      if (!logPractices.length) {
        return res
          .status(404)
          .json({
            message: "No se encontraron prácticas para este estudiante",
          });
      }
